...
class AdataState:
    def __init__(self, workspace_id, active="adata_raw"):
        ...
        self.load_adata(workspace_id=workspace_id)
    ...
    def delete_record(self, adata: AdataModel = None):
        try:
            ...
            self.conn.query(schemas.Adata).filter(schemas.Adata.adata_name == adata.adata_name).filter(schemas.Adata.work_id == adata.work_id).delete()
            self.conn.commit()
    ...
    def get_adata(self, adata_name=None) -> AdataModel:
        if not adata_name:
            query = self.conn.query(schemas.Adata).filter(schemas.Adata.work_id == st.session_state.current_workspace.id).all()
            return query
